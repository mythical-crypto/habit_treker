...
- Runtime: Node.js 20+ / localhost:3000
...
### Layout
- Source lives in: `src/app/` (Next.js App Router), `src/components/`, `src/lib/`
...
- Do not modify: `drizzle/` (generated migrations), `src/components/ui/` (shadcn generated)

### Conventions specific to this repo
- Naming: PascalCase components, camelCase utilities, kebab-case files
- Import style: Absolute via `@/` alias; group order: React → external → internal → relative
- Error handling: Server Actions return `{ success: boolean, error?: string }`; never throw in UI
- Database: Use Drizzle relational queries (`with`) for joins; raw SQL only for streak calculations
- Auth: Check session via `getUser()` from `src/lib/auth.ts`; middleware handles redirects
- Styling: Tailwind utility classes; design tokens from Stitch design system (see PROJECT_OVERVIEW.md)
- Forms: React Hook Form + Zod validation
- Components: Server Components by default; mark `'use client'` only for interactivity

### Forbidden
- `as any`, `@ts-ignore`, `@ts-expect-error` — fix types instead
- Hardcoded colors — use Tailwind theme tokens or design system colors
- 1px solid borders for sectioning — use tonal layering (surface colors)
- Pure black (`#000000`) — use `on-surface` token (#191C1D)
- Client Components for data fetching — use Server Components + Server Actions
- Managed auth services (Clerk, Auth0) — data must stay in PostgreSQL
